...
def constraints_C(model, m, non_aggregate_grouped, p):
    ...
    model.addConstr(gp.quicksum(
        m[idx] for idx, _ in enumerate(non_aggregate_grouped.items())) == person.count)

    # Person is a total of sum days absent
    model.addConstr(gp.quicksum(
        duration * m[idx] for idx, (duration, count) in enumerate(non_aggregate_grouped.items())) ==
                    person.sum)
...
def constraints_D(model, m, non_aggregate_grouped, p, rounding, k):
    ...
    p_count = model.addVar(vtype=GRB.INTEGER, name=f'count absences person {person_idx}', lb=1,
                           ub=sum(non_aggregate_grouped)-k) # Replace value depending on experiment
    ...
    model.addConstr(gp.quicksum(
        m[idx] for idx, _ in enumerate(non_aggregate_grouped.items())) == p_count)

    # Person is a total of sum days absent in current solution
    model.addConstr(gp.quicksum(
        duration * m[idx] for idx, (duration, _) in enumerate(non_aggregate_grouped.items())) == p_sum)
    ...
    if rounding is None:
        ...
        model.addConstr(gp.quicksum(
            m[idx] * ((duration - person.average) ** 2) for idx, (duration, _) in
        ...
        for idx, (duration, _) in enumerate(non_aggregate_grouped.items()):
            ...
            model.addConstr(m[idx] * tmp_diff_duration_avg_squared[idx] == tmp_weighted[idx])
...
def constraints_CD(model, m, non_aggregate_grouped, p, rounding):
    ...
    # Person is a total of sum days absent
    model.addConstr(gp.quicksum(
        duration * m[idx] for idx, (duration, _) in enumerate(non_aggregate_grouped.items())) == person.sum)
    ...
    if rounding is None:
        ...
        model.addConstr(helper_a * gp.quicksum(
            m[idx] * ((duration - (person.sum / person.count)) ** 2) for idx, (duration, _) in
...
def calculate(d):
    ...
    person_idx = d["person_idx"]
    person = list(aggregate_records.itertuples(index=False))[person_idx]
    p = person_idx, person
    ...
    with gp.Env(empty=True) as env:
        ...
        with gp.Model(env=env) as model:
            m = [model.addVar(vtype=GRB.INTEGER, name=f'how many of {i} has person {person_idx}', lb=0)
                  for i in non_aggregate_grouped.index]

            # Default constraints
            for idx, (duration, count) in enumerate(non_aggregate_grouped.items()):
                model.addConstr(m[idx] <= count)
            ...
            model.setParam("PoolSolutions", 1001)  # Calculate at most 2B solutions
            model.setParam("LogToConsole", 0)
            model.setParam("SoftMemLimit", 4)  # Limit Gurobi to 48G of memory
            ...
            # Get M^p_* per person
            person_solutions = set()
            for solN in range(model.SolCount):
                model.Params.SolutionNumber = solN
                person_solutions.add(str([int(round(value.xn)) for value in m]))

            d["n_solutions"] = n_solutions
            d["runtime"] = runtime
            d["person_solutions"] = list(person_solutions)
            model.dispose()
    return d
...
if __name__ == '__main__':
    ...
    for k in k_values:
        ...
        for index, ((QID_non_aggr, group_non_aggr), (QID_aggr, group_aggr)), in enumerate(zip(
            ...
            if QID_non_aggr != QID_aggr:
                ...
                for person_idx in range(len(group_aggr)):
    ...
    with Pool(processes=32, maxtasksperchild=32) as pool:
        for d in tqdm(pool.imap_unordered(calculate, eq_to_process), total=len(eq_to_process),
                      desc="Calculating"):
            with open(f"{output_folder}/targeted_{d['K']}_{d['test_name']}_{d['rounding']}.csv", 'a') as f:
                f.write(
                    f"""{d['QID']};{d["person_idx"]};{d["n_solutions"]};{d["runtime"]}s\n""")
